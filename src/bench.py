...
from status_list import ALG_BROTLI, ALG_ZLIB, StatusList
...
for size in sizes:
    ...
    for rev_rate in rev_rates:
        ...
        # default status list (zlib)
        statuslist = StatusList(size, 1, alg=ALG_ZLIB)
        idlist = bytearray()
        ...
        # status list brotli
        statuslist = StatusList(size, 1, alg=ALG_BROTLI)
        idlist = bytearray()
        for idx, val in enumerate(sample):
            statuslist.set(idx, val)
            if val == 1:
        ...
        rawsl_brotli = statuslist.encodeAsBytes()
